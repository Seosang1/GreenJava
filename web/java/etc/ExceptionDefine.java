...
// Exception클래스를 상속받았으므로 AgeInputException클래스는 예외클래스가 됨
// 나이입력시 유효하지 않은 값이 들어왔을 경우 발생시킬 예외클래스
	public AgeInputException() {
		super("유효하지 않은 나이가 입력되었습니다.");
		// 상위클래스인 Exception클래스의 생성자를 호출하면서 예외 발생시 보여줄 메시지를 인수로 가져감
	}
}
...
class ExceptionDefine {
	...
	// 예외처리를 현 메소드를 호출한 곳으로 넘기는 명령 - 예외처리는 try-catch문으로 해야 함
		...
		if (age < 0) {
			...
			// AgeInputException 형 인스턴스 excpt를 생성하는 것이자 AgeInputException 예외를 생성 - 예외발생아님
			throw excpt;
			// excpt에 들어있는 AgeInputException 예외를 발생했다고 JVM에 알리는 역할
			// throw excpt 가 실행되면 JVM의 예외처리 매카니즘이 동작하게 됨
		}
		return age;
	}
}

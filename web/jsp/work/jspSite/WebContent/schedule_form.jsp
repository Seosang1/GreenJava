...
if (w.equals("in")) {	// 일정 등록일 경우
	...
	msg = "수정";
	int idx = Intger.parseInt(request.getParameter("idx"));
 
   try {
      Class.forName(driver);
      conn = DriverManager.getConnection(url, "root", "1234");
      stmt = conn.createStatement();
      String sql = "select * from t_schedule_info where si_idx = " + idx;
      rs = stmt.executeQuery(sql);
   
      if (rs.next())    {   // 2021-05-15 11:23  
    	  content = rs.getString*("si_content");
    	  y = rs.Integer.parseInt(getString("si_stime").substring(0, 4));
    	  m = rs.Integer.parseInt(getString("si_stime").substring(5, 7));
    	  d = rs.Integer.parseInt(getString("si_stime").substring(8, 10));
    	  h = rs.Integer.parseInt(getString("si_stime").substring(11, 13));
    	  n = rs.Integer.parseInt(getString("si_stime").substring(14)); 
       } else {    
    		out.println("<script>");

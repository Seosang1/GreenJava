...
String pYear = "location.href='schedule2.jsp?schYear=" + 
	(sYear - 1) + "&schMonth=" + sMonth + "';";
if (sYear == 1990)	pYear = "alert('이전 연도가 없습니다.');";

String nYear = "location.href='schedule2.jsp?schYear=" + 
	(sYear + 1) + "&schMonth=" + sMonth + "';";
...
<style>
body { margin:0; }
#wrapper { width:700px; text-align:center; }
...
function isDel(idx) {
	if (confirm("정말 삭제하시겠습니까?")) {
		location.href = "schedule_proc2.jsp?w=del&idx=" + idx + "&y=<%=sYear%>&m=<%=sMonth + 1%>";
	}
}
</script>
</head>
...
<div id="wrapper">
	...
	<input type="button" value="이전 달" onclick="location.href='schedule2.jsp?schYear=<%=prevYear %>&schMonth=<%=prevMonth %>';" />
...
<% } %>
	...
	<input type="button" value="오늘" onclick="location.href='schedule2.jsp';" />
	<input type="button" value="다음 달" onclick="location.href='schedule2.jsp?schYear=<%=nextYear %>&schMonth=<%=nextMonth %>';" />
	<input type="button" value="다음 연도" onclick="<%=nYear %>" />
	</form>
	<br />
	<table width="700" class="pdt" id="col" cellpadding="5">
	<tr height="30">
...
// i : 날짜의 일(day)을 표현하기 위한 변수 / n : 일주일이 지날 때 마다 다음 줄로 내리기 위한 변수
	...
	String stime = sYear + "-" + (sMonth < 9 ? "0" + (sMonth + 1) : (sMonth + 1)) + 
		"-" + (i < 10 ? "0" + i : i);
	String sql = "select * from t_schedule_info where mi_id = '" + 
		login_info.getMi_id() + "' and si_stime like '" + stime + "%' order by si_stime";
	rs = stmt.executeQuery(sql);
	if (rs.next()) {	// 해당일에 일정이 있으면
		schImg = "<a href=\"javascript:showSchedule('box" + i + "');\">" + 
			"<img src='img/schedule_icon.png' width='20' /></a>";
%>
		<div class="scheduleBox" id="box<%=i%>">
		<%=stime.substring(0, 10)%> 일정
		<img src="img/close.png" width="20" onclick="hideSchedule('box<%=i%>');" style="cursor:pointer;" align="right" /><br />
<%
		do {
			int idx = rs.getInt("si_idx");
%>
			일시 : <%=rs.getString("si_stime").substring(11)%>&nbsp;&nbsp;
			<input type="button" value="수정" onclick="location.href='schedule_form2.jsp?w=up&idx=<%=idx%>';" />&nbsp;&nbsp;
...
<%
	...
	String lnk = "<a href='schedule_form2.jsp?y=" + sYear + "&m=" + sMonth + "&d=" + i + "&w=in'>";

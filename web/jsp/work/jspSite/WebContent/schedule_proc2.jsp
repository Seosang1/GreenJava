...
String y = null, m = null, d = null, h = null, n = null;	// 연월일시분을 저장할 변수
...
try {
	...
	if (w.equals("in")) {	// 일정 등록일 경우
		sql = "insert into t_schedule_info (mi_id, si_stime, si_content) values " + 
			"('" + login_info.getMi_id() + "', '" + stime + "', '" + content + "')";

	} else {	// 일정 수정이나 삭제일 경우
		int idx = Integer.parseInt(request.getParameter("idx"));	// 일정 번호
		if (w.equals("up")) {
			sql = "update t_schedule_info set " + 
				"si_stime = '" + stime + "', si_content = '" + content + "' " + 
				" where mi_id = '" + login_info.getMi_id() + "' and si_idx = " + idx;

		} else if (w.equals("del")) {
			sql = "delete from t_schedule_info " + 
				" where mi_id = '" + login_info.getMi_id() + "' and si_idx = " + idx;
	...
	if (result > 0) {	// 일정 추가 및 수정, 삭제 쿼리의 실행이 성공했으면
		out.println("location.href = 'schedule2.jsp" + args + "';");
	} else {

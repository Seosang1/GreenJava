...
// 게시글 목록을 받아와 연결하는 클래스
	...
	// Action 인터페이스의 abstract 메소드인 execute()를 오버라이딩하여 구현
		...
		// 현재 페이지 번호와 한 페이지에서 보여줄 게시글 개수, 한 블록에서 보여줄 페이지 개수를 저장할 변수 초기화
		...
		testInfo.setCpage(cpage);		// 현재 페이지 번호
		testInfo.setRcnt(rcnt);			// 전체 게시글 개수
		testInfo.setPcnt(pcnt);			// 전체 페이지 개수
		testInfo.setSpage(spage);		// 블록 시작 페이지 번호
		testInfo.setEpage(epage);		// 블록 종료 페이지 번호
		testInfo.setPsize(psize);		// 페이지 크기
		testInfo.setBsize(bsize);		// 블록 크기
		testInfo.setSchtype(schtype);	// 검색조건
		testInfo.setKeyword(keyword);	// 검색어

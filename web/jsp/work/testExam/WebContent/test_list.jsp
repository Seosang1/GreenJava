...
<%@ page import="vo.*"%>
<%
	ArrayList<TestInfo> articleList  = (ArrayList<TestInfo>)request.getAttribute("articleList");
	...
	// 페이징에 필요한 각종 데이터들을 testInfo형 인스턴스에 담음.
	
	int cpage = testInfo.getCpage();	// 현재 페이지 번호
	int pcnt  = testInfo.getPcnt();		// 전체 페이지 개수
	int rcnt  = testInfo.getRcnt();		// 전체 게시글 개수
	int spage = testInfo.getSpage();	// 블록 시작 페이지 번호
	int epage = testInfo.getEpage();	// 블록 종료 페이지 번호
	int psize = testInfo.getPsize();	// 페이지크기
	int bsize = testInfo.getBsize();	// 블록크기
	
	String schtype = "", keyword = "", schargs = "", args = "";
	// 검색조건, 검색어, 검색쿼리스트링, 전체쿼리스트링
	schtype = testInfo.getSchtype();	// 검색조건
	keyword = testInfo.getKeyword();	// 검색어
...
<body>
	...
	<table width="700" cellpadding="5" cellspacing="0">
		...
		<% 
			if (rcnt > 0) {	// 게시글이 있을 경우
				if (cpage == 1) {	// 현재 페이지 번호가 1이면
					...
					out.print("<a href='test_view.brd?cpage=1" + schargs + "'>");
					out.println("[&lt;&lt;]</a>&nbsp;&nbsp;");
		...
		<tr align="right"><td><a href="test_form.brd">새 글 작성</a></td></tr>
	</table> 
</body>
</html>

...
public class FoodListAdminAct implements action.Action{
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// order by 기본 셋팅
		String orderBy = " order by food_idx desc ";
		
		System.out.println("where : " + where);
		System.out.println("orderby : " + orderBy);
		  
		FoodListAdminSvc foodListAdminSvc = new FoodListAdminSvc();
		rcnt = foodListAdminSvc.getCount(where);	// 검색된 상품의 총 개수(페이지 개수를 구하기 위해 필요)
		foodList = foodListAdminSvc.getList(where, orderBy, cpage, psize);	// 현 페이지(cpage)에서 보여줄 검색된 상품목록을 받아옴
	    ...
	    forward.setPath("/admin/food/food_list.jsp");

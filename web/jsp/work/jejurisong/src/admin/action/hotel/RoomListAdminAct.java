...
public class RoomListAdminAct implements action.Action{
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		System.out.println("cpage["+cpage+"] bsize[" + bsize + "] hotel_name["+hotel_name+"] room_type["+room_type+"]");
		
		// 검색 조건에 따른 where절 생성 
		if (!comm.isEmpty(hotel_name)) {
			where += " and hotel_name = '" + hotel_name + "'";  
		...
		RoomListAdminSvc listAdminSvc = new RoomListAdminSvc();

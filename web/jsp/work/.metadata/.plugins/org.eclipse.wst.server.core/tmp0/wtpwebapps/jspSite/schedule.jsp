...
<div id="wrapper">
   ...
   <input type="button" value = "이전 년도"  onclick="<%=pYear %>" />
   <input type="button" value="이전 달" onclick = "location.href='schedule.jsp?schYear=<%=prevYear %>&schMonth=<%=prevMonth %>';"/>
...
<% } %>
   ...
   <input type="button" value="오늘" onclick= "location.href='schedule.jsp';"/>
   <input type="button" value="다음 달" onclick = "location.href='schedule.jsp?schYear=<%=nextYear %>&schMonth=<%=nextMonth %>';"/>
   <input type="button" value = "다음 년도"  onclick="<%=nYear %>" />
   </form>
   <br />
...
// i : 날짜의 일(day)을 표현하기 위한 변수 / n : 일주일이 지날 때 마다 다음 줄로 내리기 위한 변수
   ...
   String schImg = "";      // 일정이 있으면 보여줄 이미지

   if (n % 7 == 1) {   // 요일번호가 1이면
      out.println("<tr>");
      txtClass = " class=\"txtRed\"";   // 일요일 이므로 날짜 생삭 변경
   }
...
%>
      ...
      <img src="img/close.png" width="20" onclick="hideSchedule ('box<%=i%>');"
...
%>
         ...
         <input type="button" value="수정" 
         	onclick="location.href='schedule_form2.jsp?w=up&idx<%=rs.getInt("si_idx")%>';" />&nbsp;&nbsp;
